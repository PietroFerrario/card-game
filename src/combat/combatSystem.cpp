...
void CombatSystem::gainAttack(Entity& target, int amount)
{
...
DamageResult CombatSystem::dealDamage(Entity& target, int amount, bool ignoreArmor)
{
    ...
    if (ignoreArmor)
    {
        ...
        target.lowerHp(clampAmount);
        result.hpDamage = clampAmount;
    }
    else
    {

        const int armor = target.getArmor();
        const int blocked = std::min(clampAmount, armor);
        const int hpDamage = clampAmount - blocked;

        if (blocked > 0)
            target.lowerArmor(blocked);
        if (hpDamage > 0)
        {
            target.lowerHp(hpDamage);
        }
        result.blocked = blocked;
        result.hpDamage = hpDamage;
    }
    result.targetDied = isDead(target);
    return result;
}
